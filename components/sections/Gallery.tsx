      ...
      <Container>
        ...
        >
          {gallery.map((item) => (
            ...
            >
              <figure className="group relative h-full w-full overflow-hidden rounded-2xl shadow-sm shadow-foreground/5">
                <Image
                  src={item.src}
                  alt={item.alt}
                  fill
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-primary/80 via-primary/0 to-transparent opacity-70 transition-opacity duration-300 group-hover:opacity-90" />
                <figcaption className="absolute inset-x-0 bottom-0 flex flex-col gap-1 p-5 text-primary-foreground">
